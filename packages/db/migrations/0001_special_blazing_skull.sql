CREATE TABLE `signal_device_identities` (
...
--> statement-breakpoint
CREATE INDEX `signal_one_time_keys_claim_idx` ON `signal_one_time_keys` (`user_id`,`device_id`,`created_at`);--> statement-breakpoint
...
CREATE TABLE `__new_dm_channels` (
	...
	CONSTRAINT "dm_channels_encryption_scheme_check" CHECK("__new_dm_channels"."encryption_scheme" in ('legacy-ecdh', 'signal-protocol'))
);
--> statement-breakpoint
